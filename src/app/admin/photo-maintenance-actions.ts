...
// las fotos ya subidas: regenera y sobreescribe tanto las que no tienen
// pie de foto como las que ya tienen uno puesto por esta misma función o
// al subirlas (descriptionAutoGenerated = true) — así se puede relanzar
// para mejorar resultados antiguos (p. ej. tras cambiar de modelo). Las
// que el admin ha escrito o editado a mano (descriptionAutoGenerated =
// false) no se tocan nunca. Igual que reprocessAllPhotos, es una acción
// puntual pensada para lanzar cuando se quiera, no algo que se dispare solo.
export async function identifyExistingPhotos(): Promise<{
  ...
  const photos = await prisma.photo.findMany({
    where: { OR: [{ description: null }, { descriptionAutoGenerated: true }] },
    select: {
      id: true,
      thumbPath: true,
      galleryId: true,
  ...
  for (const photo of photos) {
    ...
    try {
      ...
      await prisma.photo.update({
        where: { id: photo.id },
        data: { description: caption, descriptionAutoGenerated: true },
      });
